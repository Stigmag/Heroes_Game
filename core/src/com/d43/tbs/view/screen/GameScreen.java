...
// *********************************************************** MAP
		map = new CellMap(GameScreen.textureAtlas, textureAtlas.findRegion("dirt"), -Gdx.graphics.getWidth() / 2,
	...
	private void init() {
		...
		defeatedZone = new DefeatedZone(GameScreen.textureAtlas, textureAtlas.findRegion("0"), -5f, 10f, 1f, 1f);
		...
		this.mapPlaying.setAtlas(GameScreen.textureAtlas);
...
// *********************************************************** UNITS CONTAINER
	...
	private void unitsFromFile(ArrayList<Unit> units) {
		...
		for(int i = 0; i < units.size(); i++) {
			...
			if(units.get(i).getClass().toString().equals(Archer.class.toString())) {
				unit = new Archer(GameScreen.textureAtlas.findRegion("archer"), location.x, location.y, size.width, size.height);
			} else if(units.get(i).getClass().toString().equals(Knight.class.toString())) {
				unit = new Knight(GameScreen.textureAtlas.findRegion("knight"), location.x, location.y, size.width, size.height);
			} else if(units.get(i).getClass().toString().equals(Orc.class.toString())) {
				unit = new Orc(GameScreen.textureAtlas.findRegion("orc"), location.x, location.y, size.width, size.height);
			} else if(units.get(i).getClass().toString().equals(Zombie.class.toString())) {
				unit = new Zombie(GameScreen.textureAtlas.findRegion("zombie"), location.x, location.y, size.width, size.height);
			}
			else {
				unit = new Archer(GameScreen.textureAtlas.findRegion("archer"), location.x, location.y, size.width, size.height);
			...
			units.get(i).reInit(GameScreen.textureAtlas, cell, unit.getTextureRegion(), animations.get(1), animations.get(2));
		}
	...
	public void render(float delta) {
		...
		map.draw(batch);
			...
			batch.draw(GameScreen.textureAtlas.findRegion("game_behind"), -Gdx.graphics.getWidth() / 2,
					-Gdx.graphics.getHeight() / 2);
	
			for (int i = 0; i < units.size(); i++)
				units.get(i).draw(batch, delta);
	
			batch.draw(GameScreen.textureAtlas.findRegion("choose_above"), -Gdx.graphics.getWidth() / 2,
					-Gdx.graphics.getHeight() / 2);
		batch.end();
	...
	public void setTextureAtlas(TextureAtlas textureAtlas) {
		GameScreen.textureAtlas = textureAtlas;
	}
}
